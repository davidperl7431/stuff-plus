...
years = sorted(df_scored["game_year"].dropna().unique().tolist(), reverse=True)
...
# -----------------------------
asf = arsenal_summary.copy()
asf_year = asf[asf["game_year"] == year].copy()

asf_filt = asf_year[pd.to_numeric(asf_year["n_total"], errors="coerce") >= min_total_pitches].copy()
pitcher_list = (
    asf_filt["PlayerName"]
    .dropna()
    .sort_values()
...
# -----------------------------
with tab_profile:
    # Pitch-level data first
    dfp = df_scored[
        (df_scored["game_year"] == year) &
        (df_scored["PlayerName"] == pitcher_name)
    ].copy()

    # Header summary
    row = arsenal_summary[
        (arsenal_summary["game_year"] == year) &
        (arsenal_summary["PlayerName"] == pitcher_name)
    ]

    if len(row) == 0:
        st.warning("No pitcher summary found in arsenal_summary for this selection.")
    else:
        r = row.iloc[0]
        ...
        if "is_lefty" in dfp.columns and dfp["is_lefty"].notna().any():
            ...
            is_lefty = int(r.get("is_lefty", 0)) == 1
        ...
        with c1:
            ...
            st.caption(f"pitcher_id: {int(r['pitcher']) if pd.notna(r['pitcher']) else 'NA'}")
        ...
        with c3:
            st.metric(
                f"{year} Pitches",
                f"{int(r['n_total'])}" if pd.notna(r["n_total"]) else "NA"
            )

        with c4:
            st.metric(
                "Overall Stuff+",
                f"{r['StuffPlus_pt_overall']:.0f}" if pd.notna(r["StuffPlus_pt_overall"]) else "NA"
            )

        if pd.notna(r.get("primary_pitch", np.nan)) and pd.notna(r.get("primary_pitch_usage", np.nan)):
            st.caption(f"Primary pitch: {r['primary_pitch']} ({r['primary_pitch_usage']:.0%})")
    ...
    dfp["iVB"] = pd.to_numeric(dfp["pfx_z"], errors="coerce")
    ...
    # -----------------------------
    hist = df_scored[df_scored["PlayerName"] == pitcher_name].copy()
    hist = hist[hist["game_year"].notna() & hist["pitch_type"].notna()].copy()
    hist["HB"] = pd.to_numeric(hist["HB_obs"], errors="coerce")
    hist["iVB"] = pd.to_numeric(hist["pfx_z"], errors="coerce")
    hist["ssw_in"] = pd.to_numeric(hist.get("ssw_in"), errors="coerce")
    hist["spin_efficiency"] = pd.to_numeric(hist.get("spin_efficiency"), errors="coerce")
    ...
    if len(hist) == 0:
        ...
        with col1:
            ...
            g = (
                hist.groupby(["game_year", "pitch_type"], as_index=False)
                    .size()
                    .rename(columns={"size": "Pitches", "pitch_type": "Pitch"})
            )
        ...
        hist_shape = (
            hist.groupby(["game_year", "pitch_type"], as_index=False)
                .agg(
                    ...
                    iVB=("iVB", "mean"),
                    HB=("HB", "mean"),
                    **{"Arm Angle": ("arm_angle", "mean")},
                    Extension=("release_extension", "mean"),
                    SSW=("ssw_in", "mean"),
                    **{"Spin Eff.": ("spin_efficiency", "mean")},
                    Spin=("release_spin_rate", "mean"),
                    **{"Spin Axis": ("spin_axis", "mean")},
                )
                .rename(columns={"pitch_type": "Pitch"})
        )
...
with tab_lb:
    ...
    d = df_scored[df_scored["game_year"] == year].copy()
...
with tab_finder:
    ...
    df_finder_year = df_scored[df_scored["game_year"] == year].copy()
